...
class AgeGenerator {
  getAge(birthDate) {
    ...
    //생일 안지났으면 -1
    const checkAge =
      c_month > b_month || (c_month == b_month && c_day >= b_day); //return boolean (가독성있게 풀어주... 네.맞슴다 쉬운방법으로..)
    if (checkAge == true) {
      age--;
    }
    return age;
...
class AddressGenerator {
  ...
  generateAddress() {
    ...
    const num = MyUtility.getRandomInRange(1, 100);
    return `${city} ${street}길 ${num}`;
  }
}
...
function run() {
  ...
  dataPrinter.writeToCSV(users, "user.csv");
}
run();
